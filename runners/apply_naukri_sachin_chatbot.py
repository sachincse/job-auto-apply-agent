"""Naukri Quick Apply with REAL chatbot Q&A for Sachin.

Mirrors the Kritika runner with Sachin's profile + cookies. Kept as a separate
file so both run independently — different cookies, results, screenshots — and
Kritika's runner is never touched when Sachin's is used.

Per memory `sachin_target_filter.md` Sachin targets outside-India, but Naukri
postings are largely Indian; he applies broadly here and engages selectively.

ORIGINAL DOC (still applies):
...
Flow per job (confirmed from DOM probe 2026-06-03):
  ...
  5. For each question: lookup answer from Sachin's profile, type, send
...
# Sachin's Naukri cookies: pre-existing file naukri_cookies.json (NOT the
# kritika_naukri_cookies.json one). If missing/expired the runner opens a headed
# browser and waits for the user to log in manually, then persists fresh cookies.
COOKIES_PATH = BASE_DIR / "data" / "cookies" / "naukri_cookies.json"
SHOTS_DIR = BASE_DIR / "data" / "naukri_sachin_apply"
SHOTS_DIR.mkdir(parents=True, exist_ok=True)
RESULTS_PATH = BASE_DIR / "data" / "naukri_sachin_results.json"
MANUAL_LOGIN_WAIT_SEC = 120  # browser stays open this long for user to log in if cookies invalid
...
# ─── Sachin's answer bank (matched to common Naukri chatbot questions) ───
# Patterns checked in order; FIRST match wins. Facts from CLAUDE.md form_answers
# + user_profile/credentials memories: 8yr ML at TrueBalance, CTC ₹55LPA fixed,
# expected ₹80LPA India equiv (>55 baseline), 60d notice, Bangalore, Male.
ANSWER_BANK = [
    # CTC questions (Naukri loves these)
    (r"current.*ctc.*lacs|current.*salary.*lacs|present.*ctc.*lacs", "55"),
    (r"current.*ctc.*annum|current.*ctc|present.*ctc|current.*salary", "5500000"),
    (r"expected.*ctc.*lacs|expected.*salary.*lacs", "80"),
    (r"expected.*ctc|expected.*salary", "8000000"),
    ...
    # Years of experience — Sachin = 8 yr total
    (r"years.*of.*relevant.*experience|relevant.*experience.*years", "8"),
    (r"years.*of.*experience|total.*experience.*years", "8"),
    (r"\bexperience\b.*\b(in|with)\b.*python", "8 years"),
    (r"\bexperience\b.*\b(in|with)\b.*(machine learning|ml|ai|nlp|llm)", "6 years"),
    # Location
    (r"current.*location|where.*are.*you.*currently|current.*city", "Bangalore"),
    (r"willing.*to.*relocate", "Yes"),
    (r"preferred.*location|location.*preference", "Bangalore, Remote, International"),
    # Skills / fit
    (r"why.*interested.*role|why.*this.*role|why.*you.*fit",
     "I have 8+ years of hands-on experience building production-grade GenAI/LLM systems "
     "and ML pipelines, currently leading AI engineering at TrueBalance (regulated fintech). "
     "Specialist in multi-agent orchestration (LangGraph, MCP), RAG with vector DBs, and "
     "end-to-end deployment on AWS/Azure. This role aligns with my expertise and growth ambitions."),
    # Demographics / personal — Sachin = Male
    (r"highest.*qualification|highest.*education|education.*level", "Postgraduate"),
    (r"\bgender\b", "Male"),
    (r"\bage\b", "30"),
    # Visa / work auth — for India-based postings work-auth=Yes; sponsorship-required=Yes (his targeting)
    (r"work.*authorization|authorized.*to.*work.*india", "Yes"),
    (r"visa.*sponsorship.*required", "Yes"),
    (r"authorized.*to.*work.*without.*sponsorship", "No"),
    # Generic yes/no
...
RADIO_OPTION_CANDIDATES = {
    ...
    # Experience: Sachin has 8yr — prefer 7-10/more-than-5 bands first.
    "experience": [
        "7-10 years", "More than 7 years", "More than 5 years", "5+ years",
        "8 years", "8 Years",
...
# Options that would DISQUALIFY Sachin — never click as fallback.
FORBIDDEN_FALLBACKS = {
...
def get_radio_candidates(question_text: str) -> list[str]:
    ...
    # ANY experience question → use generic experience candidates (Sachin = 8yr)
...
async def _verify_or_manual_login(page, ctx) -> bool:
    """Open Naukri and confirm we're logged in. If not, give the user a window
    to log in manually, then persist fresh cookies for next time.
    Returns True iff a valid logged-in session is established.
    """
    try:
        await page.goto("https://www.naukri.com/mnjuser/homepage", timeout=30000,
                        wait_until="domcontentloaded")
    except Exception as e:
        logger.warning(f"goto homepage failed: {str(e)[:120]}")

    await asyncio.sleep(4)
    url = page.url.lower()
    if "/mnjuser/" in url and "login" not in url:
        logger.info("✓ Naukri session restored from saved cookies")
        return True

    # Not logged in — ask user to do it manually
    print()
    print("=" * 70)
    print("⚠ Naukri cookies missing/expired. Log in as SACHIN now in the open")
    print(f"  browser window. You have {MANUAL_LOGIN_WAIT_SEC}s. I'll auto-save")
    print("  cookies once you reach the logged-in homepage.")
    print("=" * 70)
    try:
    ...
    for _ in range(MANUAL_LOGIN_WAIT_SEC // 3):
        await asyncio.sleep(3)
        u = page.url.lower()
        if "/mnjuser/" in u and "login" not in u:
            # Persist cookies for next time
            try:
                fresh = await ctx.cookies()
                COOKIES_PATH.write_text(json.dumps(fresh, indent=2))
                logger.info(f"✓ Saved fresh Naukri cookies ({len(fresh)}) to {COOKIES_PATH.name}")
            except Exception as e:
                logger.warning(f"cookie save failed: {str(e)[:80]}")
            return True
    logger.error("Manual login window expired without reaching logged-in state.")
    return False
...
async def main():
    print("=" * 70)
    print(f"NAUKRI CHATBOT APPLY — Sachin (cap {MAX_APPLIES})")
    print("=" * 70)
    ...
    async with async_playwright() as pw:
        ...
        # Verify the cookie session is still logged in. If not, give the user a
        # window to log in manually and then persist refreshed cookies.
        logged_in = await _verify_or_manual_login(search_page, ctx)
        if not logged_in:
            print("❌ Could not establish a logged-in Naukri session — aborting")
            await browser.close()
            return
        ...
        print(f"\n{'='*70}")
        print(f"NAUKRI SACHIN CHATBOT FINAL: {submitted} REAL applications")
        print(f"{'='*70}")
        await browser.close()
